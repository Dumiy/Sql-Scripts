...
DECLARE
  v_loc      dept_prof.location_id%TYPE:= &p_loc;
...
ADD CONSTRAINT c_pr_prof PRIMARY KEY(department_id);

ALTER TABLE dept_prof
DROP CONSTRAINT c_pr_prof;

ALTER TABLE emp_prof
ADD CONSTRAINT c_ex_prof FOREIGN KEY (department_id) 
    REFERENCES dept_prof;
    
ALTER TABLE emp_prof
DROP CONSTRAINT c_ex_prof;
...
DECLARE
  v_cod    NUMBER := &p_cod;
  v_nume   emp_prof.last_name%TYPE;
  v_sal    emp_prof.salary%TYPE;
BEGIN  
...
DECLARE
  v_comm_val  NUMBER(4);
  v_comm      emp_prof.commission_pct%TYPE;
BEGIN
