...
BEGIN
 IF (TO_CHAR(SYSDATE,'D') = 1) 
     OR (TO_CHAR(SYSDATE,'HH24') NOT BETWEEN 8 AND 11)
 THEN
...
--b
BEGIN
    FOR i IN (SELECT d.department_id id, MAX(department_name) dept_name, 
                     SUM(salary*(1+NVL(commission_pct,0))) sum_sal
              FROM departments d, employees e
              WHERE d.department_id = e.department_id (+)
              GROUP BY d.department_id) LOOP
        INSERT INTO info_dept_prof
        VALUES (i.id, i.dept_name, i.sum_sal);
    END LOOP;
END;
/
...
CREATE TABLE info_emp_prof (
    ...
    prenume VARCHAR2(25),
    salariu NUMBER(8,2),
    id_dept NUMBER(4) REFERENCES info_dept_prof(id)
);

--b
BEGIN
    FOR i IN (SELECT employee_id, first_name, last_name,
                     salary * (1+NVL(commission_pct,0)) sal,
                     id
              FROM emp_prof, info_dept_prof
              WHERE department_id = id) LOOP
        INSERT INTO info_emp_prof
        VALUES (i.employee_id, i.first_name, i.last_name, i.sal, i.id);
    END LOOP;
END;
/
