...
BEGIN
 IF (TO_CHAR(SYSDATE,'D') = 1) 
     OR (TO_CHAR(SYSDATE,'HH24') NOT BETWEEN 8 AND 9)
 THEN
...
--b
BEGIN
  FOR i IN (SELECT d.department_id id, MAX(department_name) nume, SUM(salary*(1+NVL(commission_pct,0))) plati
            FROM employees e, departments d
            WHERE e.department_id (+) = d.department_id
            GROUP BY d.department_id) LOOP
    INSERT INTO info_dept_prof
    VALUES (i.id, i.nume, i.plati);
  END LOOP;
END;
/
...
CREATE OR REPLACE TRIGGER trig4_prof
  AFTER DELETE OR UPDATE  OR  INSERT OF salary ON emp_prof
  FOR EACH ROW
BEGIN
  IF DELETING THEN 
...
BEGIN
  FOR i IN (SELECT employee_id, first_name, last_name, 
                   salary*(1+NVL(commission_pct,0)) sal, department_id
            FROM employees) LOOP
    INSERT INTO info_emp_prof
    VALUES (i.employee_id, i.first_name, i.last_name, i.sal, i.department_id);
  END LOOP;
END;
/
